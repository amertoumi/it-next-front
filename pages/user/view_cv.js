                ...
                <Row>
                    <Col className="col-8">
                        ...
                        <ul className="timeline" >
                            { CurrentUser.experiences.map(el => <li>
                                <h6>{el.poste}</h6>
                                <span className="small text-gray"><i
                                    className="fa  fa-calendar-alt m-2"/>{format(new Date(el.fromDateFormat), 'dd MMMM yyyy')} - {format(new Date(el.toDateFormat), 'dd MMMM yyyy')}</span>
                                <p>{el.description}</p>
                            </li>)}
                        </ul>
                        ...
                        <div className="ml-4">
                            { CurrentUser.langues.map(el =>
                                    <label className="skills_name" style={{paddingRight : '1rem'}}><b> {el.libelle} : </b> {el.criteriaLangues.map(el =>
                                        <label className="skills_name" style={{paddingRight : '1rem'}}> {el.libelle} </label>)} </label>
                               )}
                        </div>
                    </Col>
                    <Col className="col-4">
                        ...
                        { CurrentUser.educations.map(el =>
                            ...
                            <h6>{el.degree}</h6>
                                ...
                                <span className="small text-gray"><i
                                    className="fa  fa-calendar-alt m-2"/>{format(new Date(el.fromDateEdu), 'dd MMMM yyyy')} - {format(new Date(el.toDateEdu), 'dd MMMM yyyy')}</span>
                        ...
                        <div>
                            { CurrentUser.typeskills.map(el =>
                                <div>
                                    <p><b className="skills_type mt-2 mb-2">{el.type} :</b></p>
                                    {el.items.map(el =>
                                        <label className="skills_name" style={{paddingRight : '1rem'}}> {el} </label>)}
                                </div>)}
                        </div>
                    </Col>
                </Row>
